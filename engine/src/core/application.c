...
b8 application_create(game* game_inst) {
    ...
    if (!event_initialize()) {
        KERROR("Event sustem failed initialization, cannot continue");
        return FALSE;
    }
...
b8 application_on_key(u16 code, void* sender, void* listener_inst, event_context context) {
    if (code == EVENT_CODE_KEY_PRESSED) {
        ...
        if (key_code == KEY_ESCAPE) {
            ...
            // example of checking for key
            KDEBUG("Explicit - A key pressed");
        } else {
            KDEBUG("'%c' key pressed in windows.", key_code);
        }
        ...
        if (key_code == KEY_B) {
            ...
            KDEBUG("'%c' key released in windows.", key_code);
        }
    }

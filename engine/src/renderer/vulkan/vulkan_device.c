...
void vulkan_device_destroy(vulkan_context* context) {
    ...
    // Physical deveices are not actually destroyed, the resources are jsut released
...
b8 select_physical_device(vulkan_context* context) {
    ...
    if (!context->device.physical_device) {
        KERROR("No physical devices were found which match the requirements");
        return FALSE;
    }
...
b8 physical_device_meets_requirements(
    ...
    vulkan_physical_device_queue_family_info* out_queue_info,
    vulkan_swapchain_support_info* out_swapchain_support) {
    // Evaluate device properties to see if it meets the needs of our applicaiton
    out_queue_info->graphics_family_index = -1;
    out_queue_info->present_family_index = -1;
    out_queue_info->compute_family_index = -1;
    out_queue_info->transfer_family_index = -1;

    // Check if it is a discrete GPU
    ...
    if (
        ...
        if (requirements->device_extension_names) {
            ...
            if (available_extensions_count != 0) {
                available_extensions = kallocate(sizeof(VkExtensionProperties) * available_extensions_count, MEMORY_TAG_RENDERER);
                VK_CHECK(vkEnumerateDeviceExtensionProperties(
                    device,
                    0,
                    &available_extensions_count,
                ...
                for (u32 i = 0; i < required_extension_count; ++i) {
                    ...
                    if (!found) {
                        ...
                        kfree(available_extensions, sizeof(VkExtensionProperties) * available_extensions_count, MEMORY_TAG_RENDERER);
                        return FALSE;
                    }
                }
                kfree(available_extensions, sizeof(VkExtensionProperties) * available_extensions_count, MEMORY_TAG_RENDERER);
